...
def compute_hierarchical(data, k, key, rd_metric, alg_metric):
    ...
    clustering = AgglomerativeClustering(n_clusters=k, metric=alg_metric, linkage = 'average').fit(X)
    data.obs[key] = pd.Categorical(values=clustering.labels_, categories=np.unique(clustering.labels_))

    data.uns['time'][f'hierarchical_{rd_metric}_{alg_metric}'] = time.time() - start_time
    return data
...
def main(dataset_file, output_path, algorithm):
    ...
    for rd_metric in ['hvg', 'pca', 'pcoa_l1', 'pcoa_l2', 'pcoa_cosine', 'pcoa_correlation', 'isomap_l1', 'isomap_l2', 'isomap_cosine', 'isomap_correlation', 'kpca_l2', 'kpca_cosine']:
        for alg_metric in ['l1','l2','cosine','correlation']:
            ...
            data = compute_hierarchical(data, len(Counter(data.obs['label'])), 'hierarchical_' + rd_metric + '_' + alg_metric , rd_metric, alg_metric)

            entry = get_stats(data, file, 'hierarchical', 'label' , rd_metric, alg_metric, 'write_path')
