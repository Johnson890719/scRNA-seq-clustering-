...
def get_silhouette_metric(alg_metric):
    if 'correlation' in alg_metric:
        return 'precomputed'
    elif 'cosine' in alg_metric:
        return 'cosine'
...
def compute_KNN(data, rd_metric='pcoa_l2', alg_metric='l2'):
    ...
    if rd_metric == 'hvg':
        X = data.obsp[f'dm_{alg_metric}']
        X = X.toarray() if scipy.sparse.issparse(X) else X
        data.obsm[f'dm_{alg_metric}'] = X  # Assign dense matrix back to data.obsm
        sc.pp.neighbors(data, n_neighbors=10, use_rep=f'dm_{alg_metric}', metric=alg_metric, key_added=f'KNN_{alg_metric}')
    else:
...
def main(dataset_file, output_path, algorithm):
    ...
    for rd_metric in ['hvg', 'pca', 'pcoa_l1', 'pcoa_l2', 'pcoa_cosine', 'pcoa_correlation', 'isomap_l1', 'isomap_l2', 'isomap_cosine', 'isomap_correlation', 'kpca_l2', 'kpca_cosine']:
        for alg_metric in ['l1','l2','cosine','correlation']:
            ...
            data = compute_leiden(data, resolution=1.0, rd_metric=rd_metric, alg_metric=alg_metric)

            entry = get_stats(data, file, 'leiden', 'label' , rd_metric, alg_metric, 'write_path')
